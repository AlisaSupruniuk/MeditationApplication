...
class MeditationAdapter(private  val onItemListener: (item: MeditationModel) -> Unit): ListAdapter<MeditationModel, MeditationAdapter.MeditationViewHolder>(MeditationDiffCallBack()) {

    override fun onCreateViewHolder(parent: ViewGroup, viewType: Int): MeditationViewHolder =
        MeditationViewHolder(ItemMeditationCardBinding.inflate(LayoutInflater.from(parent.context), parent, false)).apply {
    ...
    class MeditationViewHolder(val binding: ItemMeditationCardBinding): RecyclerView.ViewHolder(binding.root) {
...
class MeditationDiffCallBack: DiffUtil.ItemCallback<MeditationModel>() {
    override fun areItemsTheSame(oldItem: MeditationModel, newItem: MeditationModel): Boolean = oldItem.url == newItem.url
